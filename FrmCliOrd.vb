...
#Region " EVENTI "
    ...
    Private Sub ClientiDataGrid_SelectionChanged(sender As Object, e As DataGridViewCellEventArgs) Handles ClientiDataGrid.CellClick
        ...
        Try
            ...
            Dim idCliX As Integer = CInt(ClientiDataGrid.Rows(e.RowIndex).Cells("IdClienti").Value)
            ResetFiltro(OrdiniDataGrid)
            GridFilter.FiltraDgv(OrdiniDataGrid, $"IDCliOrd = {idCliX}")
...
#Region " CLICK BUTTON "
    ...
    End Sub
#End Region
#Region "Calcola totali"
    Private Sub OrdiniDataGrid_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles OrdiniDataGrid.CellContentClick

        Try
            If OrdiniDataGrid.Rows(e.RowIndex).IsNewRow Then Exit Sub

            If e.ColumnIndex = 7 OrElse e.ColumnIndex = 6 Then
                CalcolaRiga(e.RowIndex)
            End If

        Catch ex As Exception
            RJMessageBox.Show("Errore durante il calcolo: " & ex.Message,
                              "Errore",
                              MessageBoxButtons.OK,
                              MessageBoxIcon.Error)
        End Try
